# admin アプリ - AI Agent Context

このファイルは admin アプリ(運営向け)の設計と開発フローを記載しています。
...
student アプリと同じアーキテクチャだが、bundle / deploy / 認可ルールは完全に独立。

```
apps/admin/
├── app/                          # Next.js App Router
│   ├── login/page.tsx
│   ├── (authed)/                 # 認証 + role==='admin' チェック
│   │   ├── layout.tsx
...
│   │       └── error.tsx
│   ├── api/                      # Route Handlers (Stripe webhook, Cron 等)
│   ├── layout.tsx
│   ├── page.tsx
│   └── globals.css
...
├── login/page.tsx
├── (authed)/
│   ├── layout.tsx              # 認証 + role==='admin' ガード
│   └── users/[userId]/lessons/page.tsx
├── api/
└── page.tsx
```
...
`(authed)/layout.tsx` は `data/auth.ts` の `verifySession` を呼び、`role !== "admin"` なら `/login` にリダイレクトする。
...
| アプリ内の複数 route | `apps/admin/components/<Component>.tsx` |
...
- ドメインの不変条件 (`canApprove` 等) は `model/` の純粋関数で呼ぶ
...
```tsx
// app/(authed)/users/[userId]/lessons/error.tsx
"use client"
...
新機能 (例: 運営が対象生徒のレッスンを開放する) を追加する時は、ドメイン (`model/`) → 認可付きデータアクセス (`data/`) → 画面 (`app/`) の順に下から積み上げる。
...
#### 1. `model/<entity>.ts` を整える

- 生の型 (`packages/db` の Drizzle schema から `InferSelectModel` で導出)
- DTO 型 (`Pick<...>` で公開してよいフィールドを定義) と `toXDto()` 変換関数
- 入力検証用 Zod schema (Server Action とフォームで再利用)
- ドメインの不変条件 (`canApprove` 等、引数で必要な値を全て受け取る純粋関数)
- 隣に `*.test.ts` でルールのユニットテスト

#### 2. `data/<entity>.ts` を整える

- 冒頭 `import "server-only"`
- read 関数: `verifySession` (role==='admin' を確認) → DB → 生 row 返却 (`cache()` でラップ)
- write 関数: `verifySession` (role==='admin' を確認) → DB 更新
...
- Server Component、`data/` から取得 → `toXDto()` で DTO 化 → Client Component に渡す
...
packages/db/
├── src/schema.ts
└── drizzle/0001_add_users_status.sql
...
#### 2. migration を生成 (`pnpm db:generate`)
...
`DATABASE_URL=<prod> pnpm db:migrate`。順序は **migration → deploy**。
...
```
apps/admin/
├── app/(authed)/users/UserTable.tsx   # 移動元
└── components/UserTable.tsx           # 移動先
```

型 / Zod / ルールの昇格:
```
apps/admin/
├── app/(authed)/users/role-schema.ts   # 移動元
└── model/users.ts                      # 移動先 (集約)
```
